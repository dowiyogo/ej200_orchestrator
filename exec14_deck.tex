...
\begin{frame}{F2: timing resolution vs photon yield — where more light stops helping}
  \framesubtitle{%
    Fit: $\sigma_t = \sqrt{a^2/N_{pe}+b^2}$ \;|\;
    Poisson-dominated ($b\approx0$) for $N_{pe}^\text{weak}\in[11,\,77]$}
  \begin{columns}[c]
    \begin{column}{0.48\textwidth}
      \centering
  ...
  \begin{tabular}{lccc}
    & $a$ [ps] & $b$ [ps] & $\chi^2$/ndf \\
    \textcolor{ej204col}{\textbf{EJ-204}} & $1312\pm24$ & $0$ & 5.22 \\
    \textcolor{ej230col}{\textbf{EJ-230}} & $1198\pm23$ & $0$ & 5.13 \\
  \end{tabular}
  \;—\;
  $N_{pe}^\text{weak}\!\in[11,77]$: Poisson-dominated.
  Optical-path floor $b$ (F7 RMS gap) not yet visible at $N_{pe}^\text{weak}<80$.}
\end{frame}
