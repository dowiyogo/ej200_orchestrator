...
% ══════════════════════════════════════════════════════════════════════════════
\section{$N_{pe}$ threshold and Top profiles — F2, F4}

\begin{frame}{F2: Ensemble $\sigma_t$ vs End $N_\text{pe}$ threshold}
  \framesubtitle{Both materials saturated at 100\% — threshold \emph{does not} discriminate}
  \begin{columns}[c]
    \begin{column}{0.48\textwidth}
      \centering
  ...
  \vspace{0.2em}
  {\footnotesize
    All events pass thresholds 5--25\,PE ($N_\text{pe}^\text{End}\gg25$).
    $\sigma\approx5$\,ns reflects \textbf{positional spread} across 31 positions,
    not intrinsic resolution. Per-position resolution: see F5.}
\end{frame}

\begin{frame}{F4: $\langle N_\text{pe}\rangle$ Top profiles vs position (EJ-204 EndTop)}
  \centering
  \includegraphics[width=0.80\textwidth,keepaspectratio]{fig04.pdf}
  \vspace{0.2em}

  \begin{itemize}
    \item T4 and T20 \textbf{saturated at 100\%} at all positions:
          Top never falls below 20\,PE — robust signal across the full kinematic range.
  ...
  \begin{columns}[c]
    ...
    \begin{column}{0.38\textwidth}
      ...
      {\footnotesize
        ...
        and shifts $v_\text{eff}^\text{onset}$ vs nominal.\\[0.3em]
        At $x=-690$: cluster uses IDs 16–24
        ($x_w=-692\to-532$\,mm) — truncated at the left edge; no channels further left.}
    \end{column}
  \end{columns}
\end{frame}
